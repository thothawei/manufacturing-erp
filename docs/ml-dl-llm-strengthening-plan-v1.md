...
**要關掉的限制**：ML 規劃第 10 節「沒有校準」「展示資料落在訓練分布之外」。

- 在 `ml/train.py` 加一段校準評估：先算未校準的 Brier score 與 reliability diagram 分箱數字，
  再試 Platt scaling 與 isotonic 各跑一次。**資料量只有 1200 筆，很可能結論是「不校準」** ——
  那就把數字留在文件裡寫明為什麼不做，比現在只寫「沒做」強。
- OOD 檢查落在 C# 側：訓練時把七個特徵的訓練分布邊界（p1/p99）寫進 metadata，
  `OnnxDelayRiskModel` 推論時比對，超出範圍的在回傳裡加 `outOfDistribution: true` 與哪幾個特徵超出。
  展示工單的 `weekly_load_ratio = 0.125`（訓練範圍 0.5~1.6）會是第一個被標出來的案例 ——
  **這正是要的效果**：模型不該安靜地對它沒見過的輸入給出自信的數字。

**驗收**：`/api/work-orders/{no}/delay-risk` 對展示工單回傳 OOD 標記；
新增測試釘住「特徵超界時必須標記」，把邊界檢查拔掉該測試會紅。
...
從 S0 開工（1 天、無相依、直接關掉兩條既有限制），完成後再進 S1。
