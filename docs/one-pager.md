...
> 使用者用自然語言提問，AI 只能透過九個唯讀工具查詢，**所有數字都由後端算好**。
...
2. **九個唯讀工具的 tool-use 迴圈** —— 工具契約、錯誤碼、稽核 log 都有明確設計；
   LLM 供應商被抽象在 `ILlmClient` 後面，換一家只要換一個類別。
3. **本機 RAG（第九個工具）** —— Ollama embedding + SQLite 向量檢索，
   回傳的是**段落與來源引用**，不是「答案」，因為答案從哪來必須追得到。
4. **一次真實的模型選型事故，變成了 CI 的一道把關** ——
   第一版預設 `nomic-embed-text`，255 個測試全綠，裝上真模型才發現：
   無關查詢（「今天天氣如何」）的相似度比真正相關的查詢還高，
   **沒有任何門檻值能分開兩者，防幻覺的第一道防線形同不存在**。
   換成 `bge-m3` 之後，這件事被寫成一個獨立的 CI job（接真實 Ollama，不准 skip）。
5. **每條防線都做過反向驗證** —— 把防線拔掉、確認測試會紅，再還原。
   沒有紅過的測試等於沒有測試。十二條防線的驗證結果全部列在文件裡。

## 量化成果

| | |
|---|---|
| 測試 | 255 個（本機有 Ollama 時 264），0 警告，CI 兩個 job 全綠 |
| AI 工具 | 9 個唯讀工具，背後 8 個 Application 服務 |
| 架構邊界 | 11 個架構測試守住分層依賴 |
...
LLM 要求 `run_mrp_shortage_analysis()` → 後端回「面板淨缺 130 片、建議下單 150 片」→
