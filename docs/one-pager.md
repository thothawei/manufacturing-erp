...
> 使用者用自然語言提問，AI 只能透過十個唯讀工具查詢，**所有數字都由後端算好**。
...
2. **十個唯讀工具的 tool-use 迴圈** —— 工具契約、錯誤碼、稽核 log 都有明確設計；
...
| | |
|---|---|
| 測試 | 291 個（本機有 Ollama 時 300），0 警告，CI 兩個 job 全綠 |
| AI 工具 | 10 個唯讀工具，背後 8 個 Application 服務 |
| 架構邊界 | 11 個架構測試守住分層依賴 |
