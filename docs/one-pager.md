...
| | |
|---|---|
| 測試 | 413 個（本機有 Ollama 時 443），0 警告，CI 兩個 job 全綠 |
...
- `AnthropicLlmClient` 尚未對真實 Anthropic API 打過請求（開發機沒有金鑰）；
  ...
  同一筆帳延伸到 Agent 端到端評測 harness（`AgentEvaluationTests`，44 題、
  量工具選擇正確率／數字幻覺率／拒答正確率）—— 計分邏輯本身已離線驗證過，
  但真模型在這 44 題上的實際表現同樣只差一次金鑰。
