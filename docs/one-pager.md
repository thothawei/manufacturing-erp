...
> 使用者用自然語言提問，AI 只能透過十一個定義好的工具查詢，**所有數字都由後端算好**；
...
2. **十一個工具的 tool-use 迴圈** —— 工具契約、錯誤碼、稽核 log 都有明確設計；
...
| | |
|---|---|
| 測試 | 358 個（本機有 Ollama 時 388），0 警告，CI 兩個 job 全綠 |
| AI 工具 | 11 個（10 唯讀 + 1 寫入建議），背後 9 個 Application 服務 |
| 架構邊界 | 11 個架構測試守住分層依賴 |
