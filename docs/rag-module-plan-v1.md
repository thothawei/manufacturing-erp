...
沿用 TV-100 的展示情境（面板可用 80 片、WO-01 延遲 2 天、面板淨缺 120 片），
...
- **`OllamaEmbeddingClient` 尚未對真實 Ollama 驗證過**（直到實際跑過一次為止）——
  ...
  > **實作後補註**：這條已關閉。裝了 Ollama 實跑驗證過，而且那一輪實測換掉了預設模型
  > （見 11.2 第 0 條）—— 這條「只要裝個服務就能關」的缺口，關掉它的代價不是裝服務的時間，
  > 是發現技術選擇錯了。
...
- 檢索品質的**標註問答對只有 8 組**（5 相關 + 3 無關），不是正式評測集。
  `RetrievalQualityTests` 本身已經進 CI 並實際執行（`retrieval-quality` job，
  log 顯示 `Passed: 9`），所以「有沒有在把關」不再是缺口；
  「把關得夠不夠細」仍然是 —— 8 組題目擋得住模型不可用，擋不住細微的品質下滑。

  > **後續（2026-09-13）**：評測集已擴充到 30 組並加上 MRR 與 Recall@3 基線，
  > 見 README 的「檢索品質怎麼量」。擴充過程量出一個原本沒發現的限制：
  > 「主題沾得上邊但語料沒寫」的問題（售價、付款條件）分數與真正相關題目重疊，
  > 相似度門檻分不開，防線只能放在 system prompt。
...
第二件事是**我加的快取是負收益，量完之後移掉了**。原本假設「下載 1.2 GB 模型是瓶頸」，
實測：`ollama pull bge-m3` 只花 **5 秒**（runner 在 Azure，約 240 MB/s），
而快取還原 8 秒 + 儲存 5 秒。時間真正花在安裝 Ollama（約 60 秒）與 CPU 跑 embedding（約 130 秒）。

移掉快取連帶移掉一整套只為它存在的複雜度：為了讓快取路徑正確，原本得用 `OLLAMA_MODELS`
指定模型位置 —— 因為 Linux 安裝腳本會建立 `ollama` 系統使用者、把模型放進
`/usr/share/ollama/.ollama/models`，快取 `~/.ollama/models` 會永遠是空的，
而那個錯誤的症狀只是「每次都重新下載」，不會有任何錯誤訊息。
那個陷阱本身仍然成立，只是現在沒有快取需要它了。

**這件事的順序值得記錄**：我是先假設瓶頸在下載、寫了快取與配套的路徑設定，
才去量每個步驟的時間。量完發現假設錯了，而且快取那一步讓 job 從 214 秒變成 223 秒。
如果沒有去量，它會一直留在 workflow 裡，看起來像個優化。

**仍然沒有解決的**：標註問答對只有 8 組，擋得住「模型在中文語料上不可用」這種級別的退化，
擋不住細微的品質下滑。（2026-09-13 已擴充到 30 組並加上 MRR／Recall@3 基線。）
