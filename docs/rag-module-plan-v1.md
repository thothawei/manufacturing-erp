...
| 項目 | 估算 | 依據 |
|---|---|---|
| 向量儲存空間 | 768 維 × 4 bytes × 33 ≈ 101 KB | `nomic-embed-text` 是 768 維 |
| 單次查詢的乘加次數 | 768 × 33 ≈ 25,000 | brute-force，微秒級，不需要索引 |
| 單次查詢的實際瓶頸 | Ollama embed 一次 query 的往返 | **未實測**，本機沒裝 Ollama |
| 建索引耗時 | 30 次 embed 往返 | **未實測**，同上 |
...
三件都是實作或測試時撞到的，不是事後回想（詳細敘述在 README 的「實作時踩到的坑」）：
...
- 沒裝 Ollama 時服務照常啟動：`/health` 回 200，啟動 log 印出
  「索引 0 段…（索引未建立：需要本機 Ollama 並執行 ollama pull nomic-embed-text；其他八個工具不受影響）」，
  `RagIndexBuilder` 記一筆 Warning 而不是拋例外。
- 255 個測試全綠、`dotnet format --verify-no-changes` 通過、Release 建置 0 警告。

**沒驗過**：
- `OllamaEmbeddingClient` 對真實 Ollama 的請求（開發機沒裝；請求內容已用假伺服器逐欄檢查）。
- 「檢索找得準不準」。現有測試用假 embedding（字元雜湊袋），能驗排序、門檻、引用座標、
  錯誤契約，驗不了語意品質 —— 那需要真實模型加一組人工標註問答對，屬於行為評測。
- 裝好 Ollama 之後的啟動 log（「索引 33 段」那行是依格式推出來的）。
