...
誠實的邊界：它在 CI 上是 skip 的（CI 沒有 Ollama），目前靠本機執行把關。
...
- **RAG 檢索品質的把關不在 CI 上**：`RetrievalQualityTests`（9 個）接真實 Ollama，
  本機有就跑、沒有就 skip。要進 CI 得讓 runner 每次下載 1.2 GB 模型。
  標註的問答對也只有 8 組，不是正式的評測集。
