...
| 對方問什麼 | 去哪 |
|---|---|
| 「這個工具的參數與回傳長怎樣」「為什麼要有 basis 欄位」 | [v2](ai-assistant-module-plan-v2.md) 第 2–4 節：工具契約、庫存計算基準、防幻覺機制。程式碼有九處註解指向這裡 |
...
要啟用：`brew install ollama && ollama serve`、`ollama pull nomic-embed-text`，
然後刪掉 `erp.db` 重新啟動，索引會在灌種子資料之後自動建立（7 份文件、33 個片段）。
...
LLM 會呼叫 `search_documents(query="面板色偏的判定標準", top_k=2)`，後端回的是這個：

```json
{
  "chunks": [
    {
      "text": "根本原因：允收標準是以單片判定的，但同一批面板若整批都偏向同一側，\n成品並列展示時的視覺差異會被放大。規格沒有錯，是判定規則缺了「批次一致性」這一層。\n這是規範本身的缺口，不是檢驗人員的疏失。",
      "source_name": "客訴處理紀錄 — 面板色偏批量客訴",
      "chunk_index": 2,
      "similarity": 0.5610836076867822
    },
    {
...
**這段 JSON 是實際跑出來貼上的，但用的是測試的假 embedding**（字元雜湊袋）——
真實 Ollama 模型的分數分布不同。片段文字、`source_name`、`chunk_index` 是真的。
...
查無資料是正常結果。但「索引根本沒建起來」是另一回事，那回的是錯誤：
...
**「找得準不準」沒有測試涵蓋** —— 現有測試用假 embedding，驗得了排序、門檻、契約，
驗不了語意品質。那需要真實模型加一組人工標註的問答對，屬於行為評測而不是單元測試。
...
- **`OllamaEmbeddingClient` 也尚未對真實 Ollama 驗證過**（開發機沒裝）。同樣用假伺服器
  逐欄檢查過請求，連線被拒／404／500／壞 JSON／逾時各自的訊息也都測過。
  這兩條是目前唯二「編譯過、測試過、沒真的跑過」的環節 —— 差別是這一條只需要裝一個
  本機服務就能關閉，不需要儲值。
- **RAG「找得準不準」沒有測試涵蓋**：現有測試用假 embedding（字元雜湊袋），
  驗得了排序、門檻、引用座標、錯誤契約，驗不了語意品質。
- **RAG 索引不會自動更新**：語料是編譯進程式的常數，改了要刪掉資料庫重建。
  沒有文件上傳端點 —— 那會帶出權限、病毒掃描、檔案儲存一整串與本模組無關的問題。
- **相似度門檻是經驗值**（預設 0.5，寫在設定裡）。它是唯一需要看實際語料與模型微調的
  參數，換 embedding 模型後要重新檢視。
