...
public class SystemPromptTests
{
    ...
    /// 「有檢索到東西」不等於「檢索到答案」。
    ///
    /// 這條是擴充檢索評測集時量出來的：問售價、付款條件、賠償金額這類
    /// 「主題沾得上邊、但語料裡根本沒寫」的問題，相似度落在 0.55–0.63，
    /// 與真正相關題目的區間重疊，相似度門檻分不開 ——
    /// 把門檻拉高到擋得住它們，就會同時擋掉真正相關的問題。
    /// 所以防線只能在這裡。原本的 prompt 只處理了「chunks 為空」那一種情況。
    [Fact]
    public void 必須規定檢索到相近段落但沒答到問題時也要說查不到()
    {
        Assert.Contains("有檢索到東西不等於檢索到答案", Prompt);
        Assert.Contains("不可以從相近的段落推測答案", Prompt);
    }
